...
class MY_Controller extends CI_Controller {
    ...
    protected function render($template='main') {
        ...
        $css_path = $this->controller_name . '/' . $this->action_name . '.css'; //set the path of the css file for each action
        if(file_exists(COMMONPATH . 'css/' . $css_path)) {
            // $this->data['css'] = 
        }
    ...
    protected function render_page($template='admin') {
        
    }

    protected function get_loggedin_user() {
        
        $username = $this->session->userdata('loggedin_user');
